...
Since the sequence number ranges from $0$ to $2^{32} - 1$ it will wrap around after a period of $2^{32}$. As soon as this happens, the parties are not able to distinguish fresh messages from the replay ones anymore. Thus, the application forces disconnection from the current session suggesting the reconnection for exchanging new session and authentication keys. 
