...
\[B \controls \fresh{A \sharekey{k_s, k_a} B} \]
...
$\fresh{payload_{M2}}$ is due to the fact that it contains $\fresh{N_A}$.

\begin{align}
\frac{A \believes B \believes payload_{M2}, A \believes B \controls payload_{M2}}{A \believes payload_{M2}} \tag*{postulate 3}
\end{align}

Since $A \believes payload_{M2}$, $A \believes A \sharekey{k_s, k_a} B$ because $k_s$ and $k_a$ are part of the payload.
